...
def dado_retirado():
    ...
    if len(tubo) < pegar: #caso os dados no tubo sejam insuficientes para completar a rodada, coloca todos os dados de volta
        ...
        for i in range(mesa):
            retomar = ddmesa.pop()
            tubo.append(retomar)
    for i in range(pegar): #pega os dados que precisam ser pego
        random.shuffle(tubo)
        sorteio = tubo.pop()
        dados_atuais.append(sorteio)
        ddmesa.append(sorteio)
    if mãos != 0: #se tiver dados na mão, coloco eles em dados_atuais, para ser retornado na função novamente
        for i in range(mãos):
            sort_m = ddmão.pop()
...
def tamanho():
    qt = len(tubo) #Mostra a quantidade de dados no tubo
    if qt != 0:
        print("=" * 50)
...
def cores(dadoesc): #sorteia as faces dos dados e mostra a cor que corresponde
    for i in dadoesc:
        if i == 'CPCTPC':
        ...
        faces.extend(cara)
    if faces[0] == 'P':   #add os dados que cairam passos em ddmão
...
while j < len(atual):  # enquanto j  for menor que os elementos do atual (jogadores), ele começa a rodada
    if len(tubo) == 0: #condição que mostra a mensagem do tubo vazio (não consegui colocar dentro de uma função)
        mesa = len(ddmesa)
        for i in range(mesa):
            retomar = ddmesa.pop()
            tubo.append(retomar)
    ...
    nome_jogadores[j]["pontos"] += pontos_jogador[1] #adiciona os pontos da rodada no dicionário
